...
gulp.task('lint', function () {
  return gulp.src([config.app + 'app/**/*(.js|!(*.constants.js|*.html))', config.test + 'specs/**/*.js'])
        .pipe(standard())
...
gulp.task('copy', function () {
  return es.merge(
        ...
        gulp.src(config.app + 'content/**/*.{woff,woff2,svg,ttf,eot,otf}')
            ...
            .pipe(gulp.dest(config.dist)),
        gulp.src(config.app + 'content/data.csv')
            .pipe(plumber({ errorHandler: handleErrors }))
...
gulp.task('content:prod', ['images', 'styles', 'html'], build)
...
gulp.task('build', ['clean'], function (cb) {
  runSequence(['copy', 'inject:vendor', 'ngconstant:prod'], 'inject:app', 'inject:troubleshoot', 'content:prod', cb)
})
